...
w:.asciiz "|W-"
	.align 2
possiblidade:.word 0 #guarda endere�o de possiblidades
...
syscall
printf(newLine) 
LOOPVERIFICAR_POS:#verificar se nao ha duplicados
li $s5,0
LOOPVERIFICAR_POS1:
...
JOGO:#funcao que recebe o count_jogadas em a0 e a pontuacao em a1 as linhas em a2 e as colunas em a3 esta funcao pega nas outras funcoes todas 
#e faz com que o jogo funcione tambem esta responsavel por acabar o jogo recome�a-lo e mostrar a pontuacao
addi $sp,$sp,-24
sw $s0,($sp)
sw $s1,4($sp)
sw $s2,8($sp)
...
move $s3,$a2 #linhas
move $s4,$a3 #colunas
SORTEIALOOP:#ciclo para sortear a cominacao
jal SORTEIA
lw $t0,chave
add $t0,$s0,$t0
sb $v0,($t0)
...
JOGOLOOP: #ciclo que esta sempre a ler verificar e comparar a sequencia e imprimir o tabuleiro ate a combinacao
#ser certa ou acabar as 10 jogadas
lw $a0,chave
li $v0,4
syscall
printf(newLine)
JOGOVER:#ciclo para verificar a sequencia a funcao esta sempre neste ciclo ate por uma sequencia que e valida
move $a0,$s4
jal lerS
lw $a0,insercao
move $a1,$s4
...
ENDJOGOLOOP:
move $s2,$v0
JOGOLOOP2: #ciclo para estar sempre a perguntar pela pontuacao ,acabar ou recome�ar o jogo
printf(opcoes)
la $t0,opcoes2
lb $t1,($t0)
li $v0,12
...
SORTEIA:#envia como retorno o endere�o do caracter
#chama a sorteianum recebe o numero e soma esse numero ao endere�o de memoria da possiblidades para ter um carater
	addi $sp,$sp,-4
	sw $ra,0($sp)
	jal SORTEIANUM
	lw $t0,possiblidade
...
SORTEIANUM: #sorteia um numero de 0 a 5 e retorna esse numero
	li $v0,42
...
#ler sequencia
lerS:	#funcao que le a sequencia inserida pelo utilizador e guarda em insercao em a0 recebe colunas
...
COMPARARSEQUENCIA:#(recebe como argumento em ao as colunas a1 a chave e 
#em a2 a insercao em a3 count_jogadas funcao void que guarda os valores de countR e countW no tabuleiro 
#dos count
addi $sp,$sp,-20
sw $s0,($sp)
sw $s1,4($sp)
